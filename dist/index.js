...
class NotionClient {
    ...
    getBlocks(blockId) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            do {
                const { results, next_cursor, has_more } = yield this.client.blocks.children.list({
                    block_id: blockId,
                    page_size: 100,
                ...
                for (const block of results) {
                    if ('has_children' in block && block.has_children) {
                        ...
                        const children = yield this.getBlocks(block.id);
...
const Article_1 = __nccwpck_require__(3432);
const utils_1 = __nccwpck_require__(1316);
...
const main = () => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const notionClient = new notion_1.default(notionToken);
    const response = yield notionClient.getArticles(notionDatabaseId);
    const articles = [];
    for (const page of response.results) {
        const blocks = yield notionClient.getBlocks(page.id);
    ...
    if (outDir) {
        ...
        articles.forEach((article) => {
            fs_1.default.writeFileSync(path_1.default.join(outDir, article.fileName), article.contents);
        });
    }
});
try {
...
const NotionBlockParser_1 = __nccwpck_require__(3213);
class Article {
    constructor() {
        this.fileName = '';
        this.contents = '';
    ...
    parse(data, imageDir) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const pageContents = yield this.parseBlocks(data.blocks);
            this.contents = gray_matter_1.default.stringify(pageContents, { title });
        });
    }
    ...
    parseBlocks(blocks) {
        const result = (0, NotionBlockParser_1.parseBlocks)(blocks, this.imageDir);
        return result;
    }
}
...
const parseBlocks = (blocks, imageDir, depth = 0) => __awaiter(void 0, void 0, void 0, function* () {
    let results = depth == 0 ? '\n' : '';
    for (const [i, block] of blocks.entries()) {
        ...
        switch (block.type) {
            ...
            case 'bulleted_list_item':
                results += `${indent2space}- ${parseRichTexts(block.bulleted_list_item.rich_text)}\n`;
            ...
            case 'numbered_list_item':
                results += `${indent3space}1. ${parseRichTexts(block.numbered_list_item.rich_text)}\n`;
                if (block.children == null &&
                    !isLast &&
...
\`\`\`\n\n`;
                break;
            case 'image':
                results += yield parseImage(block.image, block.id, imageDir);
                break;
            default:
        ...
        if (block.children) {
            const childContents = yield (0, exports.parseBlocks)(block.children, imageDir, depth + 1);
...
const parseImageFile = (url, blockId, imageDir) => __awaiter(void 0, void 0, void 0, function* () {
    let imagePath = '';
    if (imageDir) {
        fs_1.default.mkdirSync(path_1.default.join(imageDir, 'images'), { recursive: true });
        const extname = path_1.default.extname(new URL(url).pathname);
        const fileName = blockId + extname;
        const filePath = path_1.default.join(imageDir, 'images', fileName);
        imagePath = path_1.default.join('/images', fileName);
...
var StringDecoder = (__nccwpck_require__(8117).StringDecoder);
